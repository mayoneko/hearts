...
data class Card(val id: Int, val suit: Int, val number: Int, val strength: Int) {
    companion object {
        const val HEARTS = 0
    ...
    constructor(_cardID: Int) : this(_cardID, _cardID / 13, _cardID % 13 + 1, if (_cardID==1) 14 else _cardID % 13 + 1) {
        if (_cardID !in (0..51)) {
            throw IllegalArgumentException("cardID argument must be in 0..51")
        }
    }

    constructor(_suit: Int, _number: Int) : this(_suit * 13 + (_number - 1), _suit, _number, if (_number==1) 14 else _number) {
        if (_suit !in (0..3)) {
